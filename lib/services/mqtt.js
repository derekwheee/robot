...
module.exports = class MqttService extends Schmervice.Service {
    ...
    register(topic, action) {
        ...
        this.topics = { ...this.topics, [topic]: [...this.topics[topic], action] };
    }

    publish(topic, message, cb) {

        this.#verifyTopic(topic) && this.client.publish(topic, message, cb);
    }
    ...
    #verifyTopic(slug) {
        ...
        const [topic, action] = slug.split('/');

        try {
            return (topic in this.topics) && this.topics[topic].includes(action);
        }
        catch (e) {
            return false;
        }
